  ...
  output$wykres <- renderPlot({
    ...
    xdziesiec = x^10
     ...
     if(input$opcje == 'O3')
     {
        y=c(O3[x])
        plot(x,y) 
        fit=lm(y~x+xdwa+xtrzy+xcztery+xpiec+xszesc+xsiedem+xosiem+xdziewiec+xdziesiec)
        xv=seq(min(x),max(x),0.01)
        yv = predict(fit, list (x=xv, xdwa=xv^2, xtrzy=xv^3, xcztery=xv^4, xpiec=xv^5,xszesc=xv^6,xsiedem=xv^7,xosiem=xv^8,xdziewiec=xv^9,xdziesiec=xv^10)) 
